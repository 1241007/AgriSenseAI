...
> Primary context document for AI-assisted development. Read before touching any code.

---

## What This Project Is

AgriAI is a full-stack agricultural advisory platform. The backend is FastAPI + PostgreSQL + Redis. The frontend is Vite + React + TypeScript. ML inference runs via HuggingFace Transformers and scikit-learn, orchestrated through LangChain/LangGraph agents.
...
│   │   │   └── crop_history.py        # /farms/{id}/crop-history — CRUD + pagination
│   │   ├── db/
...
| FEAT-001 | Soil Analysis | ⬜ Pending | ⬜ Static |
| FEAT-002 | Fertilizer Suggestion | ⬜ Pending | ⬜ Static |
| FEAT-003 | Plant Disease Detection | ⬜ Pending | ⬜ Static |
| FEAT-004 | Crop Recommendation | ⬜ Pending | ⬜ Static |
| FEAT-005 | Yield Prediction | ⬜ Pending | ⬜ Static |
| FEAT-006 | Weather Forecast | ⬜ Pending | ⬜ Static |
| FEAT-009 | Feedback & Dashboard | ⬜ Pending | ⬜ Static |

Plans: `.claude/plans/` — start with `master-plan.md`.
...
| AI Orchestration | LangChain 0.2 + LangGraph 0.1 (planned) |
| ML Runtime | HuggingFace Transformers, scikit-learn (planned) |
...
### Prediction persistence (planned)
Every prediction writes a row to `predictions` table with `prediction_type` enum, `input_payload` JSONB, `output_payload` JSONB.

### Caching strategy (planned)
- Soil-based predictions: Redis TTL 24h
...
Core tables: `users`, `farms`, `soil_reports`, `crop_history`  
Planned: `predictions`, `feedback`

Migrations: `alembic upgrade head` — runs `0001` (users) then `0002` (farms, soil_reports, crop_history).
...
Currently implemented methods: `register`, `login`, `refresh`, `logout`, `me`, `updateProfile`, `createFarm`, `getFarms`, `getFarm`, `updateFarm`, `deleteFarm`, `createSoilReport`, `getSoilReports`, `deleteSoilReport`, `addCropHistory`, `getCropHistory`.
